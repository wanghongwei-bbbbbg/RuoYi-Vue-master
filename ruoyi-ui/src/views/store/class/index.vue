...
<script>
import { listClass, getClass, delClass, addClass, updateClass } from "@/api/store/class";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除类型管理编号为"' + classIds + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('store/class/export', {
        ...this.queryParams
